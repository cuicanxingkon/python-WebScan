...
from lib.Wappalyzer import Wappalyzer,WebPage
import lib.webanalyzer as webanalyzer
...
cms_finger_list = ['seacms','08cms', '1039_jxt', '1039\xe5\xae\xb6\xe6\xa0\xa1\xe9\x80\x9a', '3gmeeting',
...
def check(_id):
    with sqlite3.connect(pwd + '\data\cms_finger.db') as conn:
        cursor = conn.cursor()
...
class WhatCms:
    def __init__(self, target, file_path,check_thunder):
        self.cms = []
        self.diction = {}
...
def finger_query(url,threads):
    whatcms = WhatCms(url, 'data\cms_finger.db',threads)
    whatcms.run()
    finger_dic = whatcms.get_result()
    return finger_dic

def Wappalyzer_latest():
    wappalyzer = Wappalyzer.latest()
    return wappalyzer
def useWappalyzer(url,wappalyzer):
    try:
...
# exit(0)
def bannerscan(target_url,threads=5):
    try:
        banner_all=''
        cms_name =[]
        if re.match(r'^https?:/{2}\w.+$', target_url):
            ...
            fofa_finger = cms.run()
            banner = []
            wappalyzer = Wappalyzer_latest()

            for x in fofa_finger:
                banner.append(x)

            try:
                Wappalyzer = useWappalyzer(target_url,wappalyzer)
            ...
            for banner_tmp2 in banner:
                ...
                if banner_tmp2.lower() in cms_finger_list:
                    cms_name.append(banner_tmp2)
                    cms_name_flag = 1
            banner_all = banner_all.strip()
            if banner_all.startswith('| '):
                banner_all = banner_all[1:]
            if not cms_name_flag:
                cms_name_tmp = finger_query(target_url,threads)
                if cms_name_tmp:
                    cms_name.append(cms_name_tmp['cms_name'])
            # print("-" * 50)

        else:
            print("URL地址错误")

        return cms_name, banner_all
